...
1. `variables.tf` — declaración de variables (defaults de respaldo). Los **valores
   efectivos** están en `terraform.tfvars` (versionado): ahí se cambian
   `peso_principal`, `peso_contingencia`, zonas, `machine_type`, etc.
...
Los valores de las variables viven en **`terraform.tfvars`** (versionado), que tiene
mayor precedencia que los `default` de `variables.tf`. Para cambiar de escenario,
edita `peso_principal` y `peso_contingencia` en `terraform.tfvars` y vuelve a
`terraform apply`:
...
tráfico por variables, por eso editar `terraform.tfvars` es la vía correcta.
...
- **`terraform.tfvars` (versionado) manda sobre los `default` de `variables.tf`.**
  Editar un `default` en `variables.tf` NO cambia el valor efectivo si esa variable
  está en `terraform.tfvars`. Todos los cambios de valores (pesos, zonas,
  `machine_type`, `project_id`…) se hacen en `terraform.tfvars`.
- **`project_id` está fijado en `terraform.tfvars`** apuntando al proyecto de entrega
  (y `variables.tf` conserva un `default` de respaldo), para que `plan`/`apply` corran
  sin pedir input interactivo. Se puede sobrescribir con `-var="project_id=..."`, pero
  nunca debe quedar vacío.
- **El valor de `zona_principal`/`zona_contingencia` en `terraform.tfvars` debe
  coincidir con la zona realmente desplegada.** Si un *stockout* obligó a desplegar una
  VM en otra zona vía `-var`, hay que persistir esa zona en `terraform.tfvars`. Si la
  zona configurada difiere de la desplegada, un `apply` posterior intenta recrear el
...
Antes de `terraform init`, el revisor debe autenticar ADC y fijar el **quota
project** (de lo contrario gcloud avisa `Cannot find a quota project to add to
ADC` y el `apply` puede fallar con `quota exceeded` / `API not enabled`):

```bash
gcloud auth application-default login
gcloud auth application-default set-quota-project terraform-sen-2026   # usar el project_id real
```

El revisor clona el repo y ejecuta `terraform init`, luego `terraform plan` y
`terraform apply`. **No necesita editar ningún archivo**: `terraform.tfvars` ya viene
versionado con el `project_id` del estudiante (al que el revisor tiene rol Editor) y
el resto de valores. La cuenta del estudiante **debe estar limpia** (haber corrido
`terraform destroy`) para no provocar conflictos de nombres duplicados.
